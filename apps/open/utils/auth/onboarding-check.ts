...
export function getOnboardingRedirectUrl(missingFields: string[]): string {
  // If missing organization, start with organization setup
  if (missingFields.includes('organization') || 
      missingFields.includes('organization_name') ||
      missingFields.includes('organization_type')) {
    return '/onboarding/organization';
  }

  // If missing user profile fields, go to profile setup
  ...
  // If missing branch (for non-executives), go to branch setup
  if (missingFields.includes('branch') ||
      missingFields.includes('branch_name') ||
      missingFields.includes('branch_city') ||
      missingFields.includes('branch_country')) {
    return '/onboarding/branch';
  }

  // Default to organization setup
  return '/onboarding/organization';
}
